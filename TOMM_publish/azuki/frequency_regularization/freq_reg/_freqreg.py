...
from ._dct import dct, dct_3d, idct, idct_2d, idct_3d, idct_4d
...
class  Conv2d_FR4d_org(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, bias=True, minrate=0.1, droprate=0.001, dropspeed=-1, dynamicdrop=False, groups=1, directdrop=False, dilation=1):
        super().__init__()
    ...
    def forward(self, data):
        ...
        if self.bias != None:
            ...
            # pos = max(round(rate.item()*self.bias.shape[0]), 31)
            pos = round(rate.item()*self.bias.shape[0])
...
class Conv2d(Conv2d_FR4d_org):
    pass

# class Conv2d(Conv2d_FR4d):
#     pass

class Linear(Linear_FR2d_org):
    pass

# class Linear(Linear_FR2d):
#     pass

def countParamsAll(model):
    sumnum = 0
    maxnum = -1
    ...
    for name, layer in model.named_modules():
        try:
            num = layer.weightnum
            rate = layer.weightrate
            totalnum = totalnum + layer.IDROP.numel()
            
            type_layer = type(layer).__name__
            
            ori = layer.weight.numel()
            
            log += "=" * 20 + "\n"
            log += "layer_name: %s, type: %s, weightnum: %d, original param: %d, weightrate: %f\n" % (name, type_layer, num, ori, rate)
            log += "=" * 20 + "\n"
    ...
    log += "=" * 20 + "\n"
    log += "layer_name: %s, type: %s, weightnum: %d, original param: %d, weightrate: %f\n" % ("NA", "total", sumnum, totalnum, rate)
    log += "=" * 20 + "\n"

    return sumnum, minnum, maxnum, rate, log
